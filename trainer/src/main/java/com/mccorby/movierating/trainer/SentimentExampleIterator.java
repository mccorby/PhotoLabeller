...
public class SentimentExampleIterator implements DataSetIterator {
    ...
    private File[] positiveFiles;
    private File[] negativeFiles;
    ...
    // TODO Terrible method... refactor
    private DataSet nextDataSet(int num) throws IOException {
        //First: load reviews to String. Alternate positive and negative reviews
        int total = totalExamples();
        ...
        for( int i=0; i<num && cursor<totalExamples(); i++ ){
            if(cursor % 2 == 0){
                ...
                String review = FileUtils.readFileToString(positiveFiles[index]);
                reviews.add(review);
                positive[i] = true;
            } else {
                //Load negative review
                int negReviewNumber = cursor / 2;
                int index = Math.min(negReviewNumber, negativeFiles.length - 1);
                String review = FileUtils.readFileToString(negativeFiles[index]);
                reviews.add(review);
                positive[i] = false;
            }
            cursor++;
